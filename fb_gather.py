...
def get_comments_from_post(post_id,num_comments,num_queries,db,pagename):
	...
	comments = add_to_list_data(comments,'name',pagename)
	if(len(comments)>0):
		db.comments.insert(comments)
	
	i = 0
	while(num_comments > 25 and len(comments)>0):
		...
		i += 1
		if(next_comments == None or comments == None or i > 3):
			break
...
def insert_post_data(db,fb_ids):
#insert facebok post data into mongo db
...
pagenames = ["thenewyorkdailynews","orlandosentinel","chicagotribune","nytimes","saltlaketribune","washingtonpost"]
fb_ids = []

for pagename in pagenames:
	fb_ids.append({'pagename':pagename,'id':get_page_id(pagename)})

insert_post_data(db,fb_ids)
################# FQL EXAMPLE ##############
